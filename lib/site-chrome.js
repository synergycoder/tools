...
const GATED_PAGES = new Set(["vanity-address.html", "address-converter.html", "batch-send.html"]);
...
function injectStyle() {
  ...
  style.textContent = `
    ...
    .gt-feedback-strip{background:var(--panel);border-bottom:1px solid var(--hair)}
    .gt-feedback-strip-inner{max-width:1080px;margin:0 auto;padding:6px 20px;font-family:var(--sans);
      font-size:12px;color:var(--faint);text-align:center}
    /* The GnoSuggestions trigger mounted here should read as part of the
       sentence, not the widget's own default pill button — overridden only
       inside this strip, the footer's own copy keeps the default styling. */
    .gt-feedback-strip-inner .gnosuggest-trigger{display:inline;padding:0;border:none;
      background:none;color:var(--accent);font:inherit;text-decoration:underline;cursor:pointer}
    .gt-feedback-strip-inner .gnosuggest-trigger:hover{color:var(--ink)}
    ...
    .gt-nav a.gt-gated{opacity:.5}
    .gt-nav a.gt-gated:hover{opacity:.75}
    .gt-soon{display:inline-block;margin-left:5px;padding:1px 5px;border-radius:5px;
    ...
    .gt-net-btn:hover{background:rgba(87,198,162,.22)}
    ...
    .gt-footer-actions{display:flex;gap:8px;flex-wrap:wrap;justify-content:center}
...
function navLinksHTML() {
  ...
  return NAV_LINKS.map((l) => {
    ...
    return `<a href="${l.href}"${classes ? ` class="${classes}"` : ""}>${l.label}${badge}</a>`;
  }).join("");
}
...
// A slim, always-visible invite for feedback — the GnoSuggestions trigger
// mounted here is styled as an inline text link (see .gt-feedback-strip
// .gnosuggest-trigger below) rather than the widget's own default pill
// button, so it reads as part of the sentence, not a competing button next
// to the alpha banner's own warning.
function buildFeedbackStrip() {
  const strip = document.createElement("div");
  strip.className = "gt-feedback-strip";
  strip.innerHTML = `<div class="gt-feedback-strip-inner">Got an idea, or found something broken? <span id="gtHeaderSuggest"></span> — every tool here is shaped by feedback.</div>`;
  return strip;
}
...
function buildHeader() {
  ...
  header.innerHTML = `
    ...
    <nav class="gt-nav">${navLinksHTML()}</nav>
  `;
  navEl = header.querySelector(".gt-nav");
  return header;
}
...
export function initSiteChrome(opts = {}) {
  ...
  const feedbackStrip = buildFeedbackStrip();
  document.body.insertBefore(feedbackStrip, banner.nextSibling);
  const header = buildHeader();
  document.body.insertBefore(header, feedbackStrip.nextSibling);
  ...
  // tools instead of a separate one per project.
  window.GnoSuggestions?.init({
    appName: APP_NAME,
    mountSelector: "#gtHeaderSuggest",
    buttonText: "Suggest it",
  });
  window.GnoSuggestions?.init({
    appName: APP_NAME,
