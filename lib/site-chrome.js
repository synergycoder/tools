...
// Every tool lives at its own folder root (e.g. /wallet-scanner/, backed
// by wallet-scanner/index.html) rather than a flat *.html file — cleaner
// URLs, done pre-launch specifically because no real link/bookmark to the
// old flat paths existed yet to break. href/GATED_PAGES/data-page (on each
// index.html tool card) all key off this exact absolute-path form.
const NAV_LINKS = [
  { href: "/wallet-scanner/", label: "Wallet Scanner", icon: "/icons/wallet-scanner.svg" },
  { href: "/name-registry/", label: "Gnoland Name Registry", icon: "/icons/name-registry.svg" },
  { href: "/vanity-address/", label: "Vanity (Custom) Address Generator", icon: "/icons/vanity-address.svg" },
  { href: "/address-converter/", label: "Cosmos Address Converter", icon: "/icons/address-converter.svg" },
  { href: "/batch-send/", label: "Batch Send Tokens", icon: "/icons/batch-send.svg" },
  { href: "/nft-minter/", label: "NFT Minter", icon: "/icons/nft-minter.svg" },
  { href: "/messages/", label: "Onchain Messages", icon: "/icons/messages.svg" },
];
...
// page directly bypasses it trivially). It's for "coming soon" pacing on a
// tool still being tested, not for protecting anything sensitive — nothing
// gated here ever touches more than the local user's own browser-generated
// data. Every tool is public as of 2026-08-13 (site launch) — this set is
// deliberately empty, not removed: add an absolute folder path here (plus
// wrapping that page's interactive UI in a #toolBody and calling
// gateIfLocked() from its own init script, same as every previously-gated
// page already does) to gate a *future* tool without re-deriving this
// mechanism from scratch. Add whitelisted gno.land addresses to
// WALLET_WHITELIST below when a page actually needs gating again.
export const GATED_PAGES = new Set([]);

// Normalizes location.pathname to the same absolute-folder-with-trailing-
// slash form used throughout NAV_LINKS/GATED_PAGES, regardless of whether
// the browser is showing the bare folder URL, an explicit .../index.html,
// or (defensively) a folder path missing its trailing slash.
...
const LOGO_SVG = `<svg viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
  ...
  <rect x="13" y="6.5" width="6" height="4.5" rx="1.8" fill="none" stroke="#081015" stroke-width="2.2"/>
  <rect x="4" y="12" width="24" height="15" rx="3" fill="#081015"/>
  <rect x="3.3" y="14" width="25.4" height="2" fill="url(#gt-logo-grad)"/>
  <g stroke="url(#gt-logo-grad)" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" fill="none">
    <path d="M11,18 L6,18 L6,24.5 L11,24.5 L11,21.5 L8.5,21.5"/>
    <path d="M13,25 L13,18 L19,25 L19,18"/>
    <rect x="21" y="18" width="6" height="7" rx="2"/>
  </g>
  <defs>
    <linearGradient id="gt-logo-grad" x1="1" y1="1" x2="31" y2="31" gradientUnits="userSpaceOnUse">
      <stop style="stop-color:var(--accent)"/>
      <stop offset="1" style="stop-color:color-mix(in srgb, var(--accent) 70%, black)"/>
    </linearGradient>
  </defs>
</svg>`;

// The one page this promo is FOR — showing "go mint a Gem" while already on
// the Gems page would be redundant noise, not a call to action.
const GEMS_PATH = "/gems/";
...
// "testnet" (sapphire-1) is deliberately excluded from the site-wide
// picker -- Pearl replaced it as gno.land's active testnet, 2026-08-27.
// The NETWORKS entry itself stays intact (see its own doc comment in
// gno-rpc.js) since several tools on this site still reference
// NETWORKS.testnet directly by name; this only controls what shows up
// as a selectable option in the header picker built below.
const PICKER_EXCLUDED_KEYS = new Set(["testnet"]);
const NETWORK_OPTIONS = Object.entries(NETWORKS)
  .filter(([key]) => !PICKER_EXCLUDED_KEYS.has(key))
  .map(([key, net]) => ({ key, chainId: net.chainId, label: net.label }));
...
// Drives the network-tinted accent (yellow on testnet, green on mainnet —
// see the [data-network] blocks in injectStyle()) — set as early as
// possible, before injectStyle() even runs, so the very first paint
// already has the right accent instead of flashing the default blue.
if (activeNetworkKey) document.documentElement.setAttribute("data-network", activeNetworkKey);
...
function injectStyle() {
  ...
  style.textContent = `
    ...
    :root{
      --bg:#070c14; --bg2:#0c1420; --panel-base:#0e1826; --panel2-base:#101b2c;
    ...
    @media (prefers-color-scheme: light){
      :root:not([data-theme]){
        --bg:#f6f8fa; --bg2:#eef2f5; --panel-base:#ffffff; --panel2-base:#f2f5f8;
    ...
    /* Network-tinted accent — testnet reads yellow, mainnet reads green,
       site-wide, since every component already styles itself off
       var(--accent)/--accent-soft/--accent-line rather than a hardcoded
       color. Driven by [data-network] on <html> (set in emitNetwork()),
       independent of the light/dark [data-theme] axis above — each pairs
       with both, same dark-unguarded-default / light-override structure. */
    :root[data-network="testnet"]{
      --accent:#d9b300; --accent-soft:#d9b30016; --accent-line:#d9b30045;
    }
    :root[data-theme="light"][data-network="testnet"]{
      --accent:#8a6d00; --accent-soft:#8a6d0012; --accent-line:#8a6d0050;
    }
    @media (prefers-color-scheme: light){
      :root:not([data-theme])[data-network="testnet"]{
        --accent:#8a6d00; --accent-soft:#8a6d0012; --accent-line:#8a6d0050;
      }
    }
    :root[data-network="betanet"]{
      --accent:#3ecf6e; --accent-soft:#3ecf6e16; --accent-line:#3ecf6e45;
    }
    :root[data-theme="light"][data-network="betanet"]{
      --accent:#1a8f45; --accent-soft:#1a8f4512; --accent-line:#1a8f4550;
    }
    @media (prefers-color-scheme: light){
      :root:not([data-theme])[data-network="betanet"]{
        --accent:#1a8f45; --accent-soft:#1a8f4512; --accent-line:#1a8f4550;
      }
    }
    /* Panels get a faint wash of the current network accent too — same
       color-mix() trick as above, layered on top of the theme's own
       neutral panel color (--panel-base/--panel2-base) rather than
       replacing it, so this stays a hint, not a repaint. Unconditional
       (not inside a [data-network] block) since it only ever reads
       whatever --panel-base/--accent already resolved to. */
    :root{
      --panel: color-mix(in srgb, var(--panel-base) 94%, var(--accent) 6%);
      --panel2: color-mix(in srgb, var(--panel2-base) 91%, var(--accent) 9%);
    }
    ...
    /* Gems mint promo — sits under the (sticky) header, scrolls away with
       the page like the alpha banner above it, not fixed in place. A
       diagonal gradient sweeps through hue via one animated filter
       (cheap — a single property, no repainting multiple gradient layers)
       for the "color changing" gem-facet look; a diagonal highlight sweeps
       across on top of that for a bit of sparkle. Both respect
       prefers-reduced-motion — settles on one still frame instead. The
       whole thing is one big <a> (not just the CTA pill inside it) so
       clicking anywhere on the bar goes to /gems/. */
    a.gt-promo-banner{position:relative;overflow:hidden;display:block;
      border-bottom:1px solid var(--hair);text-decoration:none;cursor:pointer;
      background:linear-gradient(45deg,#ff6ec7 0%,#a06bff 40%,#5b8dff 70%,#3ecf6e 100%);
      animation:gt-promo-hue 9s linear infinite}
    .gt-promo-banner::before{content:"";position:absolute;inset:0;pointer-events:none;
      background:linear-gradient(100deg,transparent 32%,#ffffff4d 48%,transparent 62%);
      background-size:260% 100%;animation:gt-promo-sweep 5.5s ease-in-out infinite}
    @keyframes gt-promo-hue{0%{filter:hue-rotate(0deg)}100%{filter:hue-rotate(360deg)}}
    @keyframes gt-promo-sweep{0%{background-position:160% 0}100%{background-position:-60% 0}}
    @media (prefers-reduced-motion: reduce){
      .gt-promo-banner{animation:none}
      .gt-promo-banner::before{display:none}
    }
    .gt-promo-inner{position:relative;max-width:1080px;margin:0 auto;padding:12px 20px;
      display:flex;align-items:center;gap:14px;flex-wrap:wrap;font-family:var(--sans)}
    .gt-promo-icon{flex:none;filter:drop-shadow(0 0 5px #ffffff8c)}
    .gt-promo-text{flex:1;min-width:170px;display:flex;flex-direction:column;gap:2px;color:#fff;
      text-shadow:0 1px 3px rgba(0,0,0,.85),0 0 10px rgba(0,0,0,.35)}
    .gt-promo-text strong{font-size:17px;font-weight:800;letter-spacing:-.01em}
    .gt-promo-text span{font-size:13px;opacity:.92;font-weight:500}
    .gt-promo-cta{flex:none;display:inline-flex;align-items:center;gap:6px;background:#0a0f16;
      color:#fff;font-family:var(--mono);font-weight:700;font-size:12.5px;padding:8px 16px;
      border-radius:99px;white-space:nowrap;transition:transform .15s ease,box-shadow .15s ease}
    a.gt-promo-banner:hover .gt-promo-cta{transform:scale(1.05);box-shadow:0 4px 16px #00000040}
    ...
    .gt-net-btn:hover{background:var(--accent-line)}
    .gt-caret{display:inline-flex;align-items:center}
    .gt-caret svg{width:11px;height:7px;display:block}
    .gt-net-btn .gt-caret{color:currentColor;opacity:.9}
    ...
    .gt-pill .gt-caret{color:var(--muted)}
    .gt-wallet-caret svg{width:12px;height:8px}
...
const PROMO_GEM_SVG = `<svg width="28" height="28" viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
  <path d="M6 12L16 4l10 8-10 18L6 12Z" fill="#ffffff2e" stroke="#ffffff" stroke-width="1.6" stroke-linejoin="round"/>
  <path d="M6 12h20M11.5 12L16 4l4.5 8M12.7 12l3.3 18M19.3 12l-3.3 18" stroke="#ffffff" stroke-width="1" stroke-linejoin="round" opacity=".8"/>
</svg>`;

// Site-wide house ad for the one thing gno.tools currently wants every
// visitor to notice: /gems/. Not shown on that page itself (see the
// GEMS_PATH check at the call site) — everywhere else it's the first thing
// under the header.
function buildPromoBanner() {
  const banner = document.createElement("a");
  banner.className = "gt-promo-banner";
  banner.href = GEMS_PATH;
  banner.innerHTML = `<div class="gt-promo-inner">
    <span class="gt-promo-icon">${PROMO_GEM_SVG}</span>
    <div class="gt-promo-text">
      <strong>Mint a Gem</strong>
      <span>A free on-chain collectible for gno.land testers — GRC-721 on Pearl-1.</span>
    </div>
    <span class="gt-promo-cta">Mint now →</span>
  </div>`;
  return banner;
}

// Explicit width/height (not just viewBox) — without them an inline SVG
// falls back to the browser's default replaced-element box (300x150),
// which inside a 32px flex-centered button clips into oversized-looking
// bars instead of a neat, properly contained glyph. 18px in a 32px
// button is a deliberately airy fill ratio (~56%) for an elegant look;
// the shorter 12/20-width bars (vs. the old 14/20) and thinner 1.4
// stroke read as lighter/more refined than the old version at any size.
const MENU_ICON_SVG = `<svg width="18" height="18" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
  <path d="M4 6h12M4 10h12M4 14h12" stroke="currentColor" stroke-width="1.4" stroke-linecap="round"/>
</svg>`;

// Shared small down-chevron for every "opens a dropdown" affordance in
// the header (network picker, wallet pill, wallet-caret) — one crisp,
// consistently-sized SVG instead of a bare "▾" text glyph, which
// rendered inconsistently small (9px font-size) and low-contrast across
// the header's different contexts.
const CHEVRON_SVG = `<svg viewBox="0 0 10 6" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
  ...
  header.innerHTML = `
    <div class="gt-header-row">
      <div class="gt-brand-group">
        <a class="gt-brand" href="/">${LOGO_SVG}<span class="gt-word">gno.tools</span></a>
...
function renderNetworkPicker(container) {
  ...
  container.innerHTML = `
    <button class="gt-net-btn" id="gtNetBtn" title="Network — click to switch">
      <span class="gt-net-dot"></span>
      <span>${active ? active.label.replace(/\s*\(.*\)$/, "") : "Network"}</span>
      <span class="gt-caret">${CHEVRON_SVG}</span>
    </button>
...
function renderWallet(container) {
  ...
  if (status !== "connected") {
    ...
    container.innerHTML = `
      ...
      ${hasExtras ? `<button class="gt-theme-btn gt-wallet-caret" id="gtWalletCaret" title="Connection options" aria-label="Connection options">${CHEVRON_SVG}</button>` : ""}
      ${hasExtras ? `
  ...
  const nameLinkHTML = registeredName
    ? `<a class="gt-dd-name" href="/name-registry/?profile=${encodeURIComponent(registeredName)}" target="_blank" rel="noopener">@${escapeHtml(registeredName)} ↗</a>`
    : "";
...
async function handleConnect() {
  // Captured BEFORE connect() -- setState(connectedPatch(...)) below calls
  // syncActiveNetworkToWallet() internally, which would otherwise overwrite
  // this with whatever chain the wallet happened to already be on before we
  // get a chance to compare against it.
  const targetKey = activeNetworkKey;
  ...
  try {
    ...
    // Confirmed live: a wallet left on an unrelated chain (e.g. a
    // self-added "local" network) connects "successfully" per Adena's own
    // AddEstablish/GetAccount, but every page's own network-mismatch check
    // then blocks the actual feature -- reads to a user as "the wallet
    // doesn't connect" rather than "connected on the wrong chain."
    // Pushing the site's own selected network onto the wallet here
    // automates exactly what manually switching Adena's active network
    // first (then reconnecting) already fixes.
    const target = NETWORK_OPTIONS.find((o) => o.key === targetKey);
    if (target) await switchWalletNetwork(target.chainId);
  } catch (err) {
    clearConnected();
    clearWalletSnapshot();
...
async function tryAutoReconnect() {
  ...
  const targetKey = activeNetworkKey; // see handleConnect()'s own copy of this comment
...
// Cloudflare Web Analytics — cookie-less, no client-side fingerprinting,
// just a page-view beacon. Injected here (not duplicated in every page's
// own <head>) so every page that calls initSiteChrome() picks it up from
// one place, present or future.
function injectAnalytics() {
  ...
  script.setAttribute("data-cf-beacon", '{"token": "ce04f78f47d141d2866810d6b7fff840"}');
...
export function initSiteChrome(opts = {}) {
  ...
  if (normalizedPath() !== GEMS_PATH) {
    const promo = buildPromoBanner();
    document.body.insertBefore(promo, header.nextSibling);
  }
  ...
  window.GnoDonate?.init({
    ...
    appName: APP_NAME,
    // Without these the widget falls back to its own default (sapphire-1
    // testnet) — confirmed live: a "donation" was actually being signed as
    // worthless testnet GNOT. Pinned to mainnet-beta so a real donation is
    // real GNOT, regardless of which chain the connected wallet happens to
    // be on right now (Adena switches/prompts for gnoland1 itself).
    chainId: "gnoland1",
  ...
  // Undoes the inline <head> script's `visibility:hidden` (see the comment
  // there) now that the banner/header/footer are actually in the DOM — the
  // page was never visible with them missing, so there's nothing left to
  // shift into place. A page without that inline script (visibility never
  // hidden to begin with) just gets a harmless no-op here.
