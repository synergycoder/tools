...
const DOCONTRACT_TIMEOUT_MS = 90_000;

export async function doContract(messages, memo) {
  const payload = { messages };
  if (memo) payload.memo = memo;
  const timeout = new Promise((_, reject) =>
    setTimeout(() => reject(new Error("Adena did not respond within 90s — check for a stuck popup, or the extension's content-script bridge may need a reload/restart.")), DOCONTRACT_TIMEOUT_MS)
  );
  const res = await Promise.race([window.adena.DoContract(payload), timeout]);
