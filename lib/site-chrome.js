...
// name-registry.html and wallet-scanner.html both only know
// sapphire-1/gnoland1. Rendered inside the same wallet bubble
...
// Keeps the "which network" list (rendered in the wallet dropdown) honest
// about which entry is actually highlighted — activeNetworkKey used to
// only ever get set once, to networkOptions[0], at page load, with nothing
// ever correcting it against the WALLET's real connected chain. Confirmed
// live: a wallet connected on mainnet still showed "sapphire-1 (testnet)"
// highlighted as selected in that list (the page's default), directly
// under a pill correctly reading "MAINNET" — a real, not just confusing,
// bug: any page driving its own reads off activeNetworkKey (e.g. Wallet
// Scanner deciding which chain to scan) would have silently queried the
// WRONG chain relative to where the wallet actually was. Called every time
// the wallet's real chainId becomes known — connect, reconnect, account/
// network-change events, and the optimistic snapshot render on load.
function syncActiveNetworkToWallet() {
  if (state.status !== "connected" || !state.chainId) return;
  const opt = networkOptions.find((o) => o.chainId === state.chainId);
...
function renderWallet(container) {
  ...
  if (status !== "connected") {
    ...
    // in it) opens a small dropdown for the pre-connect network pick and/or
    // the "no Adena detected" note; it never duplicates the connect action.
    const hasExtras = adenaAvailable === false || networkOptions.length > 0;
    container.innerHTML = `
