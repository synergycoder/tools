...
function injectStyle() {
  ...
  style.textContent = `
    ...
    .gt-header-right{display:flex;align-items:center;gap:10px;flex-wrap:wrap}
    .gt-tool-controls{display:flex;gap:6px;align-items:center}
    .gt-net-btn{box-sizing:border-box;height:30px;display:inline-flex;align-items:center;
      font-family:ui-monospace,"SF Mono",Consolas,monospace;font-size:11.5px;
      padding:0 10px;border-radius:8px;border:1px solid #1c2c4280;background:#0e1826;
      color:#8fa2bd;cursor:pointer}
    .gt-net-btn:hover{color:#e7edf7;border-color:#5ee6c855}
    .gt-net-btn.gt-on{background:#5ee6c81a;color:#5ee6c8;border-color:#5ee6c855}
    .gt-wallet{position:relative;display:flex;align-items:center;min-height:36px}
    .gt-connect-btn{box-sizing:border-box;height:36px;display:inline-flex;align-items:center;
      font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;font-weight:700;
      font-size:13px;padding:0 16px;border-radius:99px;border:1px solid #5ee6c855;
    ...
    .gt-pill{box-sizing:border-box;height:36px;display:flex;align-items:center;gap:9px;
      font-family:ui-monospace,"SF Mono",Consolas,monospace;
      font-size:12.5px;padding:0 8px 0 14px;border-radius:99px;border:1px solid #1c2c4280;
      background:#0e1826;color:#e7edf7;cursor:pointer}
    .gt-pill:hover{border-color:#5ee6c855}
    .gt-pill.gt-skeleton{color:#8fa2bd;cursor:default}
    ...
    .gt-adena-note{position:absolute;top:calc(100% + 6px);right:0;white-space:nowrap;
      font-size:11px;color:#8fa2bd;background:#0e1826;border:1px solid #1c2c4280;
      border-radius:8px;padding:5px 9px;z-index:210}
    .gt-adena-note a{color:#5ee6c8}
    .gt-footer{position:fixed;left:0;right:0;bottom:0;z-index:150;margin:0;
      background:#0a0f16f2;backdrop-filter:blur(10px);border-top:1px solid #1c2c4280;
      padding:12px 20px;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}
    .gt-footer-inner{max-width:1080px;margin:0 auto;color:#5b6f8c;font-size:11.5px;text-align:center}
  ...
  header.innerHTML = `
    <div class="gt-header-row">
      ...
      <div class="gt-header-right">
        <div class="gt-tool-controls" id="gtToolControls"></div>
...
function renderWallet(container) {
  ...
  // Renders as a pill (not the pulsing button) so a returning visitor with
  // a stored connection never sees the disconnected shape flash by first —
  // same footprint as the final connected pill, just filling in.
  ...
  if (status !== "connected") {
    container.innerHTML = `
      <button class="gt-connect-btn" id="gtConnectBtn">Connect Adena</button>
      ${adenaAvailable === false ? `<div class="gt-adena-note">No Adena detected — <a href="${INSTALL_URL}" target="_blank" rel="noopener">install it</a>.</div>` : ""}
    `;
