...
const CFORD32_LOWER = "0123456789abcdefghjkmnpqrstvwxyz";

// Mirrors gno.land/p/nt/cford32/v0's PutCompact for ids < 2^34 (the
// 7-character compact form) — confirmed byte-for-byte against that
// package's real source. This is the encoding gno.land/p/nt/seqid/v0's
// ID.String() produces (e.g. id 6 -> "0000006", id 10 -> "000000a"), and
// therefore the real on-chain token-ID string for any collection built
// with that package — an increasingly standard pattern, not a one-off.
// Ids >= 2^34 would need the 13-character full encoding instead, but no
// realistic collection mints that many tokens, so it's not implemented.
function cford32Compact(id) {
  let s = "";
  for (const shift of [30, 25, 20, 15, 10, 5, 0]) {
    s += CFORD32_LOWER[Math.floor(id / 2 ** shift) % 32];
  }
  return s;
}

// collectionPath -> "int" | "string" | "compact", learned the first time
// OwnerOf actually succeeds for that collection so every id after the
// first doesn't have to try every argument form.
...
// and separately, confirmed live that the tx-indexer itself can simply
// be missing recent transactions a moment after they land, independent
// of any event-shape mismatch). OwnerOf(id) is part of the standard
// GRC721 surface and reads *current* ownership directly from the chain,
// no indexer or event-log guessing involved — confirmed live:
// gno.land/r/.../solarmint's OwnerOf(0)/OwnerOf(2)/OwnerOf(3) all
// resolved correctly, matching BalanceOf. Ids start at 0 (confirmed
// live). The loop counter `id` below is a plain sequence number, not
// itself the on-chain ID string — literalsFor() maps it to every
// plausible on-chain encoding (bare int, quoted plain-integer string,
// and seqid/cford32's compact string) since that's genuinely
// per-collection and not standardized by GRC721 at all.
export async function bruteForceOwnedTokenIds(net, collectionPath, ownerAddress, targetCount, opts = {}) {
  ...
  const literalsFor = (id) => {
    const candidates = [
      { format: "int", literal: String(id), value: String(id) },
      { format: "string", literal: `"${id}"`, value: String(id) },
      { format: "compact", literal: `"${cford32Compact(id)}"`, value: cford32Compact(id) },
    ];
    return knownFormat ? candidates.filter((c) => c.format === knownFormat) : candidates;
  };

  await mapLimit(ids, 10, async (id) => {
    if (found.length < targetCount) {
      for (const { format, literal, value } of literalsFor(id)) {
        try {
          const raw = await abciQuery(net, "vm/qeval", `${collectionPath}.OwnerOf(${literal})`);
          const m = /"(g1[a-z0-9]+)"\s*\.uverse\.address/.exec(raw || "");
          if (m) {
            if (!knownFormat) ownerOfArgFormat.set(collectionPath, format);
            // The on-chain token-ID string is `value`, not the loop's
            // plain sequence number `id` — callers (e.g.
            // fetchOwnedTokenMetadata) need the real string to look this
            // token back up, and for the "compact" format the two are
            // not the same text (id 10 -> "000000a", not "10").
            if (m[1] === ownerAddress) found.push(value);
            break;
          }
        } catch {
  ...
  return found.sort((a, b) => (a.length !== b.length ? a.length - b.length : a < b ? -1 : a > b ? 1 : 0));
}
