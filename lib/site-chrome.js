...
async function selectNetwork(key) {
  ...
  try {
    ...
    // Adena's own "changedNetwork" event (wired below) is meant to catch
    // this too, but confirmed live: it doesn't reliably fire for a switch
    // the page itself just requested via SwitchNetwork() — relying on it
    // alone left the header's chain badge stuck showing the pre-switch
    // chain indefinitely. Re-reading the account directly closes that gap
    // regardless of whether the event ever lands.
    const accountRes = await window.adena.GetAccount();
    if (accountRes.status === "success" && accountRes.data) {
      const { address, chainId } = accountRes.data;
      if (chainId !== state.chainId || address !== state.address) {
