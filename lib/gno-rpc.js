...
export const NETWORKS = {
  // Pearl (pearl-1) is gno.land's current active testnet, replacing
  // Sapphire on 2026-08-27 -- listed first so it's this object's first
  // insertion-order entry, which is what NETWORK_OPTIONS[0] in
  // site-chrome.js falls back to as the default network before any
  // stored preference or connected wallet is known. "testnet" (below)
  // is deliberately kept, not removed -- several other tools on this
  // site (Wallet Scanner, Name Registry, Batch Send, package-deploy,
  // Onchain Messages, NFT Minter) still reference NETWORKS.testnet
  // directly by name for their own unrelated reasons, so removing the
  // key would break them. It's excluded from the site-wide network
  // PICKER specifically in site-chrome.js's own NETWORK_OPTIONS filter,
  // not hidden here.
  pearl: {
...
export const CHAIN_INFO = {
  ...
  "pearl-1": { label: "Testnet (pearl-1)", short: "Pearl", rpcUrl: "https://rpc.pearl.testnets.gno.land" },
