...
function unreachableError(net, detail) {
  const err = new Error(`The gno.land RPC endpoint (${net.rpcUrl}) is not reachable right now — ${detail}. Please try again later.`);
...
// indication of what actually failed or which endpoint -- res.ok is
// checked explicitly now so a real outage produces a clear, specific
// message instead.
export async function abciQuery(net, path, dataStr, timeoutMs = 15000) {
  ...
  try {
    ...
    clearTimeout(timer);
    if (err.name === "AbortError") throw unreachableError(net, `timed out after ${timeoutMs / 1000}s`);
    throw unreachableError(net, err.message || "network error");
  }
  clearTimeout(timer);
  if (!res.ok) {
    throw unreachableError(net, `HTTP ${res.status}`);
  }
  let json;
  try {
    json = await res.json();
  } catch (err) {
    throw unreachableError(net, "returned an unexpected (non-JSON) response");
  }
