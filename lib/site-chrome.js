...
function injectStyle() {
  ...
  style.textContent = `
    ...
    .gt-menu-dropdown a{display:flex;align-items:center;gap:10px;padding:8px 10px;border-radius:8px;color:var(--muted);
      text-decoration:none;font-size:13px;line-height:1.3}
    .gt-menu-dropdown a span{flex:1}
    .gt-menu-icon{width:20px;height:20px;border-radius:5px;flex:none}
...
function navLinksHTML() {
  ...
  return NAV_LINKS.map((l) => {
    ...
    return `<a href="${l.href}"${classes ? ` class="${classes}"` : ""}>${icon}<span>${l.label}</span>${badge}</a>`;
  }).join("");
}
