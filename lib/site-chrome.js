...
function injectStyle() {
  ...
  style.textContent = `
    ...
    /* Gems mint promo — sits under the (sticky) header, scrolls away with
       ...
       vertical gradient sweeps through hue via one animated filter
       ...
       prefers-reduced-motion — settles on one still frame instead. */
    .gt-promo-banner{position:relative;overflow:hidden;border-bottom:1px solid var(--hair);
      background:linear-gradient(180deg,#ff6ec7 0%,#a06bff 40%,#5b8dff 70%,#3ecf6e 100%);
    ...
    .gt-promo-inner{position:relative;max-width:1080px;margin:0 auto;padding:11px 20px;
    ...
    .gt-promo-text{flex:1;min-width:170px;display:flex;flex-direction:column;gap:1px;color:#0a0f16}
    .gt-promo-text strong{font-size:14px;font-weight:800;letter-spacing:-.01em}
    .gt-promo-text span{font-size:11.5px;opacity:.82;font-weight:500}
    .gt-promo-cta{flex:none;display:inline-flex;align-items:center;gap:6px;background:#0a0f16;
      ...
      border-radius:99px;text-decoration:none;white-space:nowrap;transition:transform .15s ease,box-shadow .15s ease}
    .gt-promo-cta:hover{transform:scale(1.05);box-shadow:0 4px 16px #00000040}
...
function buildPromoBanner() {
  ...
  banner.innerHTML = `<div class="gt-promo-inner">
    ...
    <a class="gt-promo-cta" href="${GEMS_PATH}">Mint now →</a>
  </div>`;
  return banner;
}
